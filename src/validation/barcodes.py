...
from rdkit import Chem, rdmolops
from scipy import spatial
from sklearn.metrics.pairwise import euclidean_distances
from sklearn.neighbors import KDTree


def coords_to_complex(mol: Chem.Mol, use_rips: False):
    if use_rips:
        adj = rdmolops.GetAdjacencyMatrix(mol)
        dists = np.array([x.GetPositions() for x in mol.GetConformers()])
        dists = np.array([spatial.distance_matrix(x, x) for x in dists])
        dists = [np.multiply(adj, x) for x in dists]
        for dist in dists:
            rc = gd.RipsComplex(distance_matrix=dist).create_simplex_tree(
                max_dimension=3
            )
        ...
        dists = np.array([x.GetPositions() for x in mol.GetConformers()])
        for dist in dists:
            ac = gd.AlphaComplex(points=dist).create_simplex_tree()


# Below DTM-related functions taken from gudhi's TDA-tutorial;
# original code by Vincent Rouvreau: https://github.com/GUDHI/TDA-tutorial/blob/master/utils/utils_dtm.py
...
def WeightedRipsFiltrationValue(p, fx, fy, d, n=10):
    ...
    if p == np.inf:
        ...
        if d < (abs(fx**p - fy**p)) ** (1 / p):
            ...
            value = np.sqrt(((fx + fy) ** 2 + d**2) * ((fx - fy) ** 2 + d**2)) / (
                2 * d
            )
        else:
            Imin = fmax
...
def AlphaDTMFiltration(X, m, p, dimension_max=2, filtration_max=np.inf):
    ...
    Output:
        st (gd.SimplexTree): the alpha-DTM filtration.
    """
    N_tot = X.shape[0]
    ...
    st = gd.SimplexTree()  # creates an empty simplex tree
