...
# Side-chain enumeration helpers (`_backbone_atom_set`, `_side_chain_group`,
# `_compute_window_downstream_sets`) moved to `src/proposers.py` — they are
# now consumed exclusively by the proposer factories that also live there.
...
# ---------------------------------------------------------------------------
# Back-compat re-exports — proposer factories live in src/proposers.py.
# Imports placed at the bottom of mcmm.py so the helpers consumed by
# proposers (`_ordered_macrocycle_atoms`, `enumerate_backbone_windows`) are
# defined before proposers.py loads, preventing a partial-import error.
# ---------------------------------------------------------------------------
